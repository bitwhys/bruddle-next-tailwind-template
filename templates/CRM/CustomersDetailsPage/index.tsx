      ...
      {mounted && isTablet ? (
        <>
          <div className="mb-12 pt-4 text-center">
            <div className="relative mx-auto mb-2.5 h-[5.25rem] w-[5.25rem]">
          ...
          <Tabs className="mb-6" classButton="md:grow" items={typeTasks} value={type} setValue={setType} />
          {type === 'details' && <Profile />}
          {type === 'orders' && <OrdersHistoryTablet items={ordersHistory} />}
          {type === 'delivery' && <DeliveryAndPayments />}
        </>
      ) : (
        <div className="flex pt-4">
          <div className="w-[18rem] shrink-0 4xl:w-[14.68rem]">
            <Profile />
          </div>
