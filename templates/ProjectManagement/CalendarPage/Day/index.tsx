...
const Day = ({}: DayProps) => {
  ...
  return (
    <div className="card">
      <div className="-mr-0.25 border-b border-r border-n-1 px-3 pb-3 pt-5 text-center text-xs font-bold dark:border-white/40">
        Monday, 3
      </div>
          ...
          <div className="grow" key={index}>
            {item.hours.map((hour: any, index: number) => (
              <div className={classValueTime} key={index}>
                {hour.task && <Task time={hour.time} item={hour.task} />}
              </div>
            ))}
          </div>
        ))}
