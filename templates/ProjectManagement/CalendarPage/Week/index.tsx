...
const Week = ({}: WeekProps) => {
  ...
  return (
    <div className="card">
      <div className="flex border-b border-n-1 pl-12 dark:border-white/40">
        <div className="w-[calc(100%/7)] px-3 pb-3 pt-5 text-right text-xs font-bold md:px-2 md:text-center">
          Mon, 3
        </div>
        <div className="w-[calc(100%/7)] px-3 pb-3 pt-5 text-right text-xs font-bold md:px-2 md:text-center">
          Tue, 4
      ...
      <div className="-mb-0.25 -mr-0.25 flex">
        <div className="w-12">
            ...
            <div className="w-[calc(100%/7)]" key={index}>
              {item.hours.map((hour: any, index: number) => (
                <div className={classValueTime} key={index}>
                  {hour.task && (
                    <Task className="md:h-6 md:justify-center md:px-0" classTitle="md:hidden" item={hour.task} />
                  )}
                </div>
              ))}
            </div>
