...
const Card = ({ item }: CardProps) => (
  <div className="card -ml-0.25 flex items-end justify-between px-5 py-4">
    <div className="mr-4">
      <div className="mb-6 flex items-center text-sm">
        <div className="flex items-center">
          <div className="mr-1.5 w-4.5 rounded-full text-0 dark:bg-white">
            <Image className="w-full" src={item.currencyLogo} width={18} height={18} alt={item.currencyTitle} />
          </div>
      ...
      <div className={`flex items-center text-sm font-bold ${item.progress > 0 ? 'text-green-1' : 'text-pink-1'}`}>
        <Icon
          className={`mr-1 ${item.progress > 0 ? 'fill-green-1' : 'fill-pink-1'}`}
          name={item.progress > 0 ? 'arrow-up-right' : 'arrow-down-right'}
        />
        {item.progress > 0 ? '+' + item.progress : item.progress}%
      </div>
    </div>
    <div className="my-1 ml-auto h-16 max-w-[8.25rem] grow">
